def text(lstData):
    ...
    Info_Measure, Info_Change, Info_Danger, \
        Info_Crit, Info_Ice1, Info_Ice2 = [], [], [], [], [], []
    block_list = {'=91': Info_Measure, '=92': Info_Change, '=93': Info_Danger,
    ...
    for i in range(0, len(lstData), 1):
        if type(lstData[i]) == str:
            if lstData[i] in block_list.keys():
                ...
                while str(lstData[i+1])[0] != '=':              #PROBLEM HERE
                    List_to_write.append(lstData[i+1])
                    i += 1
    return Info_Measure, Info_Change, Info_Danger, \
           Info_Crit, Info_Ice1, Info_Ice2
